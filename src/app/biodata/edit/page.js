...
const defaultFormData = {
  personal: {
    fullName: '', fatherName: '', motherName: '', dateOfBirth: '', birthPlace: '',
    maritalStatus: 'single', numberOfChildren: 0, height: '', weight: '',
    complexion: '', bloodGroup: '', physicalDisability: 'None', nationality: 'Bangladeshi',
  },
  religion: {
    madhab: 'hanafi', praysFiveTimes: false, wearsPardah: false, hasBeard: false,
    avoidsHaram: false, quranRecitation: 'partial', followsSunnahDress: false,
  },
  ...
  family: {
    fatherAlive: true, fatherOccupation: '', motherAlive: true, motherOccupation: '',
    numberOfBrothers: 0, numberOfSisters: 0, familyType: 'nuclear',
    familyStatus: 'middle', familyReligiousness: 'moderate',
  },
  address: {
    permanentDivision: '', permanentDistrict: '', permanentUpazila: '',
    currentDivision: '', currentDistrict: '', currentUpazila: '', grewUpIn: 'village',
  },
  ...
  partnerExpectations: {
    ...
    district: [], education: [], profession: [], maritalStatus: [], otherExpectations: '',
  },
};
...
function mergeWithDefaults(defaults, loaded) {
  ...
  for (const section of Object.keys(defaults)) {
    ...
    // Fix boolean fields that may come as strings or undefined
    if (section === 'religion') {
      const boolFields = ['praysFiveTimes', 'wearsPardah', 'hasBeard', 'avoidsHaram', 'followsSunnahDress'];
      for (const f of boolFields) {
        result[section][f] = Boolean(result[section][f]);
      }
    }
    if (section === 'family') {
      result[section].fatherAlive = result[section].fatherAlive !== false;
      result[section].motherAlive = result[section].motherAlive !== false;
    }
    // Ensure number fields are proper numbers or empty strings (not null/undefined → NaN)
    if (section === 'personal') {
      if (!result[section].height && result[section].height !== 0) result[section].height = '';
      if (!result[section].weight && result[section].weight !== 0) result[section].weight = '';
      if (!result[section].numberOfChildren) result[section].numberOfChildren = 0;
    }
    if (section === 'education') {
      if (!result[section].passingYear) result[section].passingYear = '';
    }
    if (section === 'family') {
      if (!result[section].numberOfBrothers) result[section].numberOfBrothers = 0;
      if (!result[section].numberOfSisters) result[section].numberOfSisters = 0;
    }
    if (section === 'partnerExpectations') {
      if (!result[section].ageMin) result[section].ageMin = '';
      if (!result[section].ageMax) result[section].ageMax = '';
      if (!Array.isArray(result[section].district)) result[section].district = [];
      if (!Array.isArray(result[section].education)) result[section].education = [];
      if (!Array.isArray(result[section].profession)) result[section].profession = [];
      if (!Array.isArray(result[section].maritalStatus)) result[section].maritalStatus = [];
    }
    if (section === 'lifestyle') {
      if (!Array.isArray(result[section].hobbies)) result[section].hobbies = [];
    }
    // Fix dateOfBirth — backend stores as ISO string, input[type=date] needs YYYY-MM-DD
    if (section === 'personal' && result[section].dateOfBirth) {
      try {
        const d = new Date(result[section].dateOfBirth);
        if (!isNaN(d.getTime())) {
          result[section].dateOfBirth = d.toISOString().split('T')[0];
        }
      } catch {}
    }
  }
...
export default function EditBiodataPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (data?.biodata) {
          setFormData(mergeWithDefaults(defaultFormData, data.biodata));
        }
      } catch {
        toast.error('বায়োডেটা লোড করা যায়নি।');
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      await biodataAPI.update(formData);
      toast.success('বায়োডেটা আপডেট হয়েছে! পর্যালোচনার পরে অনুমোদিত হবে।');
      router.push('/biodata');
    } catch (error) {
      toast.error(error.response?.data?.messageBn || error.response?.data?.message || 'আপডেট ব্যর্থ হয়েছে।');
    } finally {
      setLoading(false);
    }
  };

  const InputField = ({ label, section, field, type = 'text', placeholder = '', required = false, ...props }) => (
    <div>
  ...
  const SelectField = ({ label, section, field, options, required = false }) => (
    <div>
      <label className="label">{label}{required && <span className="text-red-500 ml-1">*</span>}</label>
      <select
        value={formData[section][field] ?? ''}
  ...
  const CheckField = ({ label, section, field }) => (
    <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-gray-100 hover:bg-gray-50 transition-colors">
      <input
        type="checkbox"
        checked={Boolean(formData[section][field])}
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1:
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {formData.personal.maritalStatus !== 'single' && (
              <InputField label="সন্তান সংখ্যা" section="personal" field="numberOfChildren" type="number" placeholder="0" min={0} />
            )}
            <InputField label="উচ্চতা (সেমি)" section="personal" field="height" type="number" placeholder="যেমন: 165" min={100} max={250} />
            <InputField label="ওজন (কেজি)" section="personal" field="weight" type="number" placeholder="যেমন: 55" min={30} max={200} />
      ...
      case 3:
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <SelectField label="সর্বোচ্চ শিক্ষাগত যোগ্যতা" section="education" field="highestLevel" required
              options={Object.entries(educationLabels.bn).map(([v, l]) => ({ value: v, label: l }))} />
            ...
            <InputField label="পাসের বছর" section="education" field="passingYear" type="number" placeholder="যেমন: 2020" min={1980} max={2030} />
            <SelectField label="পেশা" section="profession" field="occupationType"
              options={Object.entries(professionLabels.bn).map(([v, l]) => ({ value: v, label: l }))} />
      ...
      case 4:
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {['জীবিত', 'মৃত'].map((opt, idx) => (
                  <label key={opt} className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      name="fatherAlive"
                      checked={formData.family.fatherAlive === (idx === 0)}
                      onChange={() => update('family', 'fatherAlive', idx === 0)}
                ...
                {['জীবিত', 'মৃত'].map((opt, idx) => (
                  <label key={opt} className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      name="motherAlive"
                      checked={formData.family.motherAlive === (idx === 0)}
                      onChange={() => update('family', 'motherAlive', idx === 0)}
                      className="accent-[#1a5276]"
                    />
                    <span className="text-sm">{opt}</span>
                  </label>
            ...
            <InputField label="ভাইয়ের সংখ্যা" section="family" field="numberOfBrothers" type="number" min={0} />
            <InputField label="বোনের সংখ্যা" section="family" field="numberOfSisters" type="number" min={0} />
            <SelectField label="পরিবারের ধরন" section="family" field="familyType" options={[
              { value: 'nuclear', label: 'একক পরিবার' }, { value: 'joint', label: 'যৌথ পরিবার' },
            ]} />
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div>
                ...
                <textarea
                  value={formData.lifestyle.aboutSelf || ''}
                  onChange={(e) => update('lifestyle', 'aboutSelf', e.target.value)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      type="number"
                      placeholder="থেকে"
                      value={formData.partnerExpectations.ageMin ?? ''}
                      onChange={(e) => setFormData((p) => ({ ...p, partnerExpectations: { ...p.partnerExpectations, ageMin: e.target.value === '' ? '' : Number(e.target.value) } }))}
                      className="input-field"
                      min={18}
                      max={80}
                    />
                    <input
                      type="number"
                      placeholder="পর্যন্ত"
                      value={formData.partnerExpectations.ageMax ?? ''}
                      onChange={(e) => setFormData((p) => ({ ...p, partnerExpectations: { ...p.partnerExpectations, ageMax: e.target.value === '' ? '' : Number(e.target.value) } }))}
                      className="input-field"
                      min={18}
                      max={80}
                    />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setFormData((p) => ({ ...p, partnerExpectations: { ...p.partnerExpectations, otherExpectations: e.target.value } }))}
    ...
    return (
      <DashboardLayout>
        <div className="max-w-3xl mx-auto flex items-center justify-center h-64">
          <FaSpinner className="animate-spin text-[#1a5276] text-3xl" />
        </div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout>
      <div className="max-w-3xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-800">বায়োডেটা সম্পাদনা</h1>
          <p className="text-gray-500 text-sm mt-1">তথ্য পরিবর্তন করুন — সংরক্ষণ করলে পুনরায় পর্যালোচনা হবে</p>
        </div>
        ...
        <div className="card">
          ...
          <div className="flex justify-between mt-8 pt-5 border-t border-gray-100">
            ...
            {currentStep < steps.length ? (
              ...
              >
                {loading ? <><FaSpinner className="animate-spin" /> সংরক্ষণ হচ্ছে...</> : <><FaCheckCircle /> পরিবর্তন সংরক্ষণ করুন</>}
              </button>
            )}
          </div>
        </div>
